  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        \draw[->, very thick, jedy_alert, bend left=35]
          (L.south) to
...
\begin{frame}{Backward Pass : Graphe Annoté}
  \centering
  \begin{tikzpicture}[
    nd/.style={rectangle, draw=jedy_mid, fill=jedy_light, rounded corners=2pt,
               minimum width=1.1cm, minimum height=0.55cm, font=\small\bfseries},
    op/.style={circle, draw=jedy_blue, fill=jedy_blue!12,
               minimum size=0.65cm, font=\small\bfseries},
    fw/.style={->, thick, color=jedy_blue!70},
    bw/.style={->, thick, color=jedy_alert, dashed},
    vl/.style={font=\tiny, color=jedy_blue},
    gr/.style={font=\tiny, color=jedy_alert},
  ]
    \node[nd] (x)  at (0.0,  0.5) {$x$};
    \node[nd] (y)  at (0.0, -1.4) {$y$};
    \node[op] (pl) at (2.1,  0.5) {$+$};
    \node[nd] (a)  at (4.0,  0.5) {$a$};
    \node[op] (mu) at (5.8,  0.5) {$\times$};
    \node[nd] (b)  at (7.6,  0.5) {$b$};

    % Valeurs forward (bleu, au-dessus)
    \node[vl, above=1pt of x] {$\mathsf{val}=2$};
    \node[vl, above=1pt of y] {$\mathsf{val}=-3$};
    \node[vl, above=1pt of a] {$\mathsf{val}=-1$};
    \node[vl, above=1pt of b] {$\mathsf{val}=3$};

    % Gradients (rouge, en-dessous)
    \node[gr, below=1pt of x] {$\mathsf{grad}=-3$};
    \node[gr, below=1pt of y] {$\mathsf{grad}=-4$};
    \node[gr, below=1pt of a] {$\mathsf{grad}=-3$};
    \node[gr, below=1pt of b] {$\mathsf{grad}=1$};

    % Flèches forward
    \draw[fw] (x) -- (pl);
    \draw[fw] (y) to[out=20, in=230] (pl);
    \draw[fw] (y) to[out=5, in=280] (mu);
    \draw[fw] (pl) -- (a);
    \draw[fw] (a) -- (mu);
    \draw[fw] (mu) -- (b);

    % Flèches backward (dashed, de droite à gauche)
    \draw[bw] (b.south) to[bend left=40]
      node[below, midway, font=\tiny, color=jedy_alert] {$\times y=-3$}
      (a.south);
    \draw[bw] (b.south) to[out=220, in=-5]
      node[pos=0.5, below, font=\tiny, color=jedy_alert] {$\times a=-1$}
      (y.east);
    \draw[bw] (a.south) to[bend left=40]
      node[below, midway, font=\tiny, color=jedy_alert] {$\times 1$}
      (x.south);
    \draw[bw] (a.south) to[out=250, in=15]
      node[pos=0.45, right, font=\tiny, color=jedy_alert] {$\times 1$}
      (y.north);
  \end{tikzpicture}
  ...
  \begin{columns}[T]
    \begin{column}{0.18\textwidth}
      \begin{block}{\small Init}
        \small $\nabla_b = 1$
      \end{block}
    \end{column}
    \begin{column}{0.40\textwidth}
      \begin{alertblock}{\small Nœud $\times$ : $b = a \cdot y$}
        \small
        $\nabla_a = 1 \cdot y = \mathbf{-3}$\\[2pt]
        $\nabla_y \mathrel{+}= 1 \cdot a = -1$
      \end{alertblock}
    \end{column}
    \begin{column}{0.38\textwidth}
      \begin{alertblock}{\small Nœud $+$ : $a = x + y$}
        \small
        $\nabla_x = -3 \cdot 1 = \mathbf{-3}$\\[2pt]
        $\nabla_y \mathrel{+}= -3 \cdot 1 = -3$\\[1pt]
        \textcolor{jedy_alert}{$\Rightarrow \nabla_y = -4$}
      \end{alertblock}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Software Design : La Dataclasse \texttt{Value}}
  \begin{columns}[T]
    \begin{column}{0.34\textwidth}
      \begin{block}{Conteneur de données pur}
        \begin{itemize}\small
          \item \code{data} : valeur numérique
          \item \code{grad} : gradient (init. à 0)
          \item \code{\_prev} : parents dans le graphe
          \item \code{\_backward} : fermeture du gradient local
        \end{itemize}
      \end{block}
      \smallskip
    \end{column}
...
x = Value(2.0)
y = Value(-3.0)
print(x)  # Value(data=2.0, grad=0.0)
      \end{lstlisting}
    \end{column}
  \end{columns}
\end{frame}
    ...
    \begin{column}{0.34\textwidth}
      \begin{block}{Pattern factory}
        \begin{enumerate}\small
          \item Calcule \code{out.data}
          \item Définit \code{\_bwd} : règle locale
          \item Assigne \code{out.\_backward}
          \item Retourne \code{out}
        \end{enumerate}
      \end{block}
      \smallskip
...
\begin{frame}{Boucle d'Entraînement}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{block}{Algorithme}
        \smallskip
        \begin{tabular}{@{}ll}
          \textbf{Entrée} & \textit{model}, $\X$, $\y$, $\lr$, $T$ \\[0.3em]
          \multicolumn{2}{@{}l}{\textbf{Répéter} $T$ \textbf{fois} :} \\[0.2em]
          \hspace{1.5em} & $\forall\, p \in \mathit{params}$ :
                            $p.\mathit{grad} \leftarrow 0$ \\[0.2em]
          \hspace{1.5em} & $\hat{\y} \leftarrow \mathit{model}(\X)$ \\[0.2em]
          \hspace{1.5em} & $\mathcal{L} \leftarrow \mathit{loss}(\hat{\y},\, \y)$ \\[0.2em]
          \hspace{1.5em} & $\mathit{backward}(\mathcal{L})$ \\[0.2em]
          \hspace{1.5em} & $\forall\, p \in \mathit{params}$ :
                            $p.\mathit{data} \leftarrow p.\mathit{data} - \lr \cdot p.\mathit{grad}$ \\[0.5em]
          \multicolumn{2}{@{}l}{\textbf{Retourner} \textit{model}}
        \end{tabular}
        \smallskip
      \end{block}
      \smallskip
      \begin{alertblock}{Zero grad obligatoire}
        \small Les gradients \textbf{s'accumulent} (\code{+=}) ---
        réinitialiser avant chaque forward
      \end{alertblock}
    \end{column}
    \begin{column}{0.41\textwidth}
      \begin{block}{4 étapes}
        \begin{enumerate}
          \item \textbf{Zero grad} : remise à zéro
          \item \textbf{Forward} : calcul de la perte
          \item \textbf{Backward} : gradients
          \item \textbf{Update} : descente de gradient
        \end{enumerate}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{Récapitulatif}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Limites \& Non-Linéarité}
        \small\begin{itemize}
          \item XOR : non linéairement séparable $\Rightarrow$ couches cachées
          \item Sans activation : $N$ couches $=$ 1 couche linéaire
          \item ReLU, Sigmoïde : brisent la linéarité
        \end{itemize}
      \end{block}
      \smallskip
      \begin{exampleblock}{Graphe de Calcul \& Chain Rule}
        \small\begin{itemize}
          \item Toute fonction $\Rightarrow$ DAG de nœuds élémentaires
          \item $\frac{\partial\Loss}{\partial x} = \frac{\partial\Loss}{\partial y} \cdot \frac{\partial y}{\partial x}$
          \item Un seul backward $\Rightarrow$ gradients de \textbf{tous} les paramètres
        \end{itemize}
      \end{exampleblock}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{alertblock}{Micro-Autograd}
        \small\begin{itemize}
          \item \code{Value} : dataclasse pure — \code{data}, \code{grad}, \code{\_prev}, \code{\_backward}
          \item Factories (\code{add}, \code{mul}\ldots) : définissent \code{\_bwd}
          \item Compositions (\code{sigmoid}, \code{mse}\ldots) : gradient \textbf{gratuit}
        \end{itemize}
      \end{alertblock}
      \smallskip
      \begin{block}{Boucle d'Entraînement}
        \small\begin{enumerate}
          \item \textbf{Forward} + loss
          \item \textbf{Zero grad}
          \item \textbf{Backward}
          \item \textbf{Update} : \code{p.data -= lr * p.grad}
        \end{enumerate}
      \end{block}
    \end{column}
  \end{columns}
