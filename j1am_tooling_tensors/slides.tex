...
\begin{frame}{Notion de Vraisemblance}
  \begin{block}{Idée fondatrice du ML supervisé}
    On cherche les paramètres $\w$ qui rendent les données \textbf{les plus probables} :
    \[
      \w^* = \arg\max_{\w}\; p(\dataset \mid \w)
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \[
        \ell(\w) = \log p(\dataset \mid \w) = \sum_i \log p(y_i \mid \bm{x}_i, \w)
      \]
    ...
    \begin{column}{0.48\textwidth}
      \begin{exampleblock}{Lien avec la MSE}
        ...
        \[
          \Loss(\w) = \frac{1}{m}\sum_{i=1}^{m}(y_i - \hat{y}_i)^2
        \]
      \end{exampleblock}
    \end{column}
  \end{columns}
