  ...
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      ...
      \begin{block}{Modèle \& Prédiction}
        ...
        \small $\hat{t}_n = \operatorname{sign}(f)
        \in \{+1,-1\}$ \quad
        Encodage $t_n \in \{+1,-1\}$ : \textbf{crucial}
        pour la loss (pas $\{0,1\}$ !).
      \end{block}
    \end{column}
...
\begin{frame}[fragile]{Algorithme Online de Rosenblatt}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
\begin{lstlisting}
def rosenblatt(X, t, lr=0.1, n_epochs=50):
    N, D = X.shape
    ones  = np.ones((N, 1))
...
\begin{frame}{De 2 Classes à $K$ Classes --- Softmax}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \vspace{-3pt}
      \textbf{Problème :} $K > 2$ classes (chiffres 0--9...).

      \begin{block}{Encodage One-Hot}
        \small $\bm{t}_n \in \{0,1\}^K$, une seule composante à $1$.
        Ex.~classe~2 sur~4~: $[0,0,1,0]$.
      \end{block}

      \begin{block}{Modèle : $K$ scores}
        \[ \bm{f}_\Theta(\bm{x}_n) = \W\bm{x}_n + \bm{b} \in \R^K \]
        $\W \in \R^{K \times D}$ : une ligne par classe.
      \end{block}

      \begin{block}{Softmax $\to$ probabilités}
        \[ p_k = \frac{e^{f_k}}{\sum_j e^{f_j}} \in (0,1),
           \quad \sum_k p_k = 1 \]
        \small Readout : $\hat{k} = \arg\max_k\, p_k$.
      \end{block}
    \end{column}
    \begin{column}{0.46\textwidth}
      \begin{block}{Cross-Entropy Catégorielle (CCE)}
        \[
          \Loss_{\text{CCE}} = -\frac{1}{N}\sum_{n}\sum_{k}
          t_{n,k}\, \log(p_{n,k})
        \]
        \small Comme $t_{n,k} \in \{0,1\}$, seul le log de la
        \textbf{vraie classe} contribue à chaque terme.
      \end{block}

      \smallskip
      \begin{block}{Gradient --- résultat remarquable}
        \[ \nabla_{f_k} \Loss_{\text{CCE}} = p_k - t_k \]
        \small Erreur = proba prédite $-$ label one-hot.
      \end{block}

      \smallskip
      {\small \textbf{PyTorch :} \code{nn.CrossEntropyLoss()}
      (Softmax + CCE en une passe).}
    \end{column}
  \end{columns}
\end{frame}
