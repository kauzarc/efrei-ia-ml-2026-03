...
\begin{frame}{Un Perceptron par Classe}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \textbf{Idée :} $K$ perceptrons indépendants, un par classe.

      \begin{block}{Score de la classe $k$}
        \[ f_k(\bm{x}_n) = \bm{w}_k\T\bm{x}_n + b_k \in \R,
           \quad \bm{w}_k \in \R^D,\ b_k \in \R \]
      \end{block}

      \begin{block}{Softmax $\to$ probabilités}
        \[ p_k = \frac{e^{f_k}}{\sum_j e^{f_j}} \in (0,1),
           \quad \sum_k p_k = 1 \]
        \small Lisse et différentiable : adaptée à GD.
      \end{block}

      \begin{block}{Encodage \& Prédiction}
        \small $\bm{t}_n \in \{0,1\}^K$ (one-hot), une composante à $1$.\\
    ...
    \begin{column}{0.44\textwidth}
      \begin{tikzpicture}[
        ...
        \node[font=\tiny, jedy_mid, align=center]
          at (2.8, -0.55) {Softmax puis $\hat{k}=\arg\max$};
      \end{tikzpicture}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \begin{alertblock}{Empilement $\Rightarrow$ 1 équation matricielle}
        \[
          \bm{f}_n
          = \underbrace{\W}_{\R^{K\times D}}\bm{x}_n
          + \underbrace{\bm{b}}_{\R^K},
          \qquad W_{k,\cdot} = \bm{w}_k\T
        \]
        \small La \textbf{ligne} $k$ de $\W$ est le perceptron de classe $k$.
      \end{alertblock}
