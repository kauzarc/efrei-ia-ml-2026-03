  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{alertblock}{Objectif minimal}
        Battre la baseline de l'intervenant : le modèle
        «~prédire la moyenne~».
      \end{alertblock}
    \end{column}
  \end{columns}
      ...
      \begin{block}{Fichiers fournis}
        \begin{itemize}
          ...
          \item \textbf{test.csv :} données sans étiquettes ---
                vos prédictions ici
          \item \textbf{sample\_submission.csv :} format attendu
        \end{itemize}
      \end{block}
      \smallskip
      \begin{block}{Format de soumission}
        Un fichier CSV avec deux colonnes :
        \texttt{id} et \texttt{Age}.
      \end{block}
    \end{column}
    \begin{column}{0.44\textwidth}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      \begin{block}{Choix du modèle}
        \begin{itemize}
          \item \textbf{Baseline :} régression linéaire
    ...
    \begin{column}{0.52\textwidth}
      \begin{block}{Accès à la compétition}
        \begin{enumerate}
          ...
          \item Rejoignez la compétition via le lien fourni
          \item Téléchargez \texttt{train.csv} et \texttt{test.csv}
          \item Première soumission : la baseline
        \end{enumerate}
      \end{block}
      \smallskip
      \begin{block}{Organisation suggérée}
        \begin{itemize}
          ...
          \item \textbf{J6 matin :} modèles avancés, tuning,
                soumission finale avant 09h00
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.44\textwidth}
      \begin{exampleblock}{Par où commencer ?}
        \begin{enumerate}
          \item Charger les données, afficher \texttt{.describe()}
          \item Encoder \textbf{Sex}, vérifier les NaN
          \item Entraîner une \textbf{régression linéaire}
          \item Soumettre --- avoir un score de référence
