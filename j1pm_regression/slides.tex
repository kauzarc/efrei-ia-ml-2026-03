...
\begin{frame}{Fonction de Coût — La MSE}
  \begin{block}{Mean Squared Error (MSE)}
    \[
      \Loss(\Theta) = \frac{1}{N} \sum_{i=1}^{N} \bigl(f_\Theta(\bm{x}_i) - y_i\bigr)^2
      = \frac{1}{N} \norm{\hat{\y} - \y}^2
    \]
    \centering
...
\begin{frame}{Gradient de la MSE}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \textbf{Calcul de} $\dfrac{\partial \Loss}{\partial w_j}$ :

      \medskip
      \[
        \begin{aligned}
          \frac{\partial \Loss}{\partial w_j}
          &= \frac{1}{N}\sum_{i=1}^N \frac{\partial}{\partial w_j}
             \bigl(\w\T\bm{x}_i - y_i\bigr)^2 \\[0.4em]
          &= \frac{1}{N}\sum_{i=1}^N 2\bigl(\w\T\bm{x}_i - y_i\bigr)
             \cdot \frac{\partial\, \w\T\bm{x}_i}{\partial w_j} \\[0.4em]
          &\quad\text{or } \w\T\bm{x}_i = \textstyle\sum_k w_k x_{i,k}
             \implies \frac{\partial\,\w\T\bm{x}_i}{\partial w_j} = x_{i,j} \\[0.4em]
          &= \frac{2}{N}\sum_{i=1}^N \bigl(\hat{y}_i - y_i\bigr)\, x_{i,j}
        \end{aligned}
      \]
    \end{column}
    \begin{column}{0.44\textwidth}
      ...
      \bigskip
      \begin{alertblock}{}
        MSE convexe en $\w$ $\Rightarrow$ \textbf{un seul minimum global}.
      \end{alertblock}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Solution Analytique — Équation Normale}
