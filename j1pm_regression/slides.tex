...
\begin{frame}{Apprentissage Supervisé — Les Objets Mathématiques}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \bigskip
      \textbf{Modèle paramétrique} $f_\Theta$
      \[
      ...
      \[
        \Theta^* = \arg\min_\Theta\; \Loss(\Theta, \X, \y)
      \]
    \end{column}
    \begin{column}{0.44\textwidth}
      \begin{tikzpicture}[
        ...
        \draw[->, jedy_alert, thick, bend right=60]
          (loss) to node[left, font=\scriptsize]{$\nabla\Loss$} (model);
      \end{tikzpicture}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Régression — Choix du Modèle}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \textbf{Tâche :} prédire une sortie \textbf{continue} $y \in \R$.

      \bigskip
      \textbf{Modèle linéaire} ($D=1$) :
      \[
        f_{w,b}(x) = wx + b
      \]

      \textbf{Modèle linéaire} (multivarié) :
      \[
        f_{\w,b}(\bm{x}) = \w\T\bm{x} + b = \sum_{j=1}^D w_j x_j + b
      \]

      \textbf{Modèle polynomial} (feature map $\Phi$) :
      \[
    ...
    \begin{column}{0.46\textwidth}
      \begin{tikzpicture}
        \begin{axis}[
          ...
          title={\small Données et modèle linéaire},
          ...
          \node[jedy_alert, font=\scriptsize] at (axis cs:3.5, 7.6)
            {$\hat{y}=wx+b$};
        \end{axis}
      \end{tikzpicture}
    \end{column}
...
\begin{frame}{Fonction de Coût — La MSE}
  \begin{block}{Mean Squared Error (MSE) — aussi appelée Least Squares}
    \[
      \Loss(\w) = \frac{1}{N} \sum_{i=1}^{N} \bigl(\hat{y}_i - y_i\bigr)^2
      = \frac{1}{N} \sum_{i=1}^{N} \bigl(f_\w(\bm{x}_i) - y_i\bigr)^2
    \]
  \end{block}

  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \textbf{Pourquoi le carré ?}
      \begin{itemize}
        \item Pénalise plus fortement les \textbf{grandes erreurs}
        \item Différentiable partout $\Rightarrow$ gradient bien défini
        \item Lien avec la vraisemblance gaussienne (vu ce matin)
      \end{itemize}

      \bigskip
      \textbf{Forme matricielle} (plus efficace) :
      \[
        \Loss(\w) = \frac{1}{N} \norm{\X\w - \y}^2
      \]
      avec $\X \in \R^{N \times D}$, $\w \in \R^D$, $\y \in \R^N$.
    \end{column}
    \begin{column}{0.46\textwidth}
      \begin{tikzpicture}
        \begin{axis}[
          ...
          axis lines=left,
          tick label style={font=\tiny},
          title={\small Loss en 1D — convexe !},
          title style={font=\small},
          ymin=0,
        ]
          \addplot[jedy_blue, thick, domain=-2:4, samples=80]
            {(x-1)^2 + 0.5};
          \addplot[jedy_alert, dashed, mark=*, mark size=2pt,
                   mark options={jedy_alert, solid}]
            coordinates {(1, 0.5)};
          \node[jedy_alert, font=\scriptsize] at (axis cs:1.8, 0.7)
            {$w^* = \arg\min \Loss$};
        \end{axis}
      \end{tikzpicture}
    \end{column}
...
\begin{frame}{Gradient de la MSE — Calcul Étape par Étape}
  On dérive $\Loss(\w) = \frac{1}{N}\sum_{i=1}^N (\w\T\bm{x}_i - y_i)^2$ par rapport à $w_j$ :
  ...
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      ...
      \[
        ...
        = \frac{1}{N} \sum_{i=1}^N
          \underbrace{2(\hat{y}_i - y_i)}_{\text{erreur}}
          \cdot
          \underbrace{\frac{\partial \hat{y}_i}{\partial w_j}}_{= x_{i,j}}
      \]

      \[
        \frac{\partial \Loss}{\partial w_j}
        = \frac{2}{N} \sum_{i=1}^N (\hat{y}_i - y_i)\, x_{i,j}
      \]
    \end{column}
    \begin{column}{0.46\textwidth}
      \textbf{Forme vectorielle} (tous les $w_j$ d'un coup) :
      \[
        \boxed{
          \grad_\w \Loss = \frac{2}{N}\, \X\T (\X\w - \y)
        }
      \]
      \begin{itemize}
        \item $\X\w - \y \in \R^N$ : vecteur des erreurs
        \item $\X\T(\cdot) \in \R^D$ : pondère par les features
      \end{itemize}
    \end{column}
  \end{columns}

  \bigskip
  \begin{alertblock}{Propriété cruciale pour la suite}
    La MSE linéaire est \textbf{convexe} en $\w$ $\Rightarrow$ \textbf{un seul minimum global}.\\
    La descente de gradient est garantie de le trouver (avec un bon $\lr$).
  \end{alertblock}
\end{frame}
...
# Gradient MSE augmentee
grad = (2/N) * X_aug.T @ (y_hat - y)
\end{lstlisting}
      ...
      \begin{tikzpicture}
        \begin{axis}[
          ...
          \addplot[only marks, mark=triangle*, mark size=3pt, jedy_alert]
            coordinates {(2.1, 0.87)(5.2, 0.92)};
          % global min
          \addplot[only marks, mark=star, mark size=4pt, jedy_example]
            coordinates {(7.8, 0.55)};
          \node[font=\scriptsize, jedy_alert]   at (axis cs:2.1, 1.3) {local};
          \node[font=\scriptsize, jedy_example] at (axis cs:7.8, 1.0) {global};
        \end{axis}
      \end{tikzpicture}
    \end{column}
  \end{columns}
