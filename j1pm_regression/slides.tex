...
\begin{frame}{Régression Linéaire — Récapitulatif du Problème}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      ...
      \[
        \hat{y}_i = f_{\w,b}(\bm{x}_i) = \w\T\bm{x}_i + b
      \]
      On cherche $\w \in \R^D$ et $b \in \R$ qui \textbf{ajustent} cette droite aux données.

      \medskip
      \textbf{Objectif} — minimiser l'erreur quadratique moyenne :
      \[
        \operatorname{argmin}_{\w,\, b}\; \Loss(\hat{\y},\, \y)
        \;\Longleftrightarrow\;
...
\begin{frame}{Prédictions : du scalaire au matriciel}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \textbf{Forme scalaire} — une prédiction à la fois :
      \[
        \hat{y}_i = \w\T\bm{x}_i, \quad i = 1, \ldots, N
      \]

      \begin{exampleblock}{Forme compacte}
        \[
          \hat{\y} = \X\w, \qquad \X \in \R^{N \times D}
        \]
        \textbf{Un seul produit matriciel} pour les $N$ prédictions.
      \end{exampleblock}
    \end{column}
    \begin{column}{0.44\textwidth}
      \begin{center}
      \begin{tikzpicture}[
        mat/.style={draw=jedy_blue, fill=jedy_light,
                    minimum width=1.4cm, minimum height=0.48cm,
                    font=\scriptsize, inner sep=2pt},
        vec/.style={draw=jedy_mid, fill=jedy_mid!20,
                    minimum width=0.7cm, minimum height=0.48cm,
                    font=\scriptsize, inner sep=2pt},
        res/.style={draw=jedy_example, fill=jedy_example!15,
                    minimum width=0.8cm, minimum height=0.48cm,
                    font=\scriptsize, inner sep=2pt},
      ]
        % Matrice X
        \foreach \i/\t in {0/$\bm{x}_1\T$, 1/$\bm{x}_2\T$, 2/$\vdots$, 3/$\bm{x}_N\T$} {
          \node[mat] at (0, -\i*0.58) {\t};
        }
        \node[above, font=\small\bfseries, text=jedy_blue] at (0, 0.32) {$\X$};
        \node[below, font=\tiny, text=gray] at (0, -3*0.58-0.3) {$N \times D$};

        % Vecteur w
        \foreach \i/\t in {0/$w_1$, 1/$w_2$, 2/$\vdots$, 3/$w_D$} {
          \node[vec] at (2.0, -\i*0.58) {\t};
        }
        \node[above, font=\small\bfseries, text=jedy_mid] at (2.0, 0.32) {$\w$};
        \node[below, font=\tiny, text=gray] at (2.0, -3*0.58-0.3) {$D \times 1$};

        % Opérateurs
        \node[font=\large] at (1.45, -0.87) {$\cdot$};
        \node[font=\large] at (2.8, -0.87) {$=$};

        % Vecteur ŷ
        \foreach \i/\t in {0/$\hat{y}_1$, 1/$\hat{y}_2$, 2/$\vdots$, 3/$\hat{y}_N$} {
          \node[res] at (3.65, -\i*0.58) {\t};
        }
        \node[above, font=\small\bfseries, text=jedy_example] at (3.65, 0.32) {$\hat{\y}$};
        \node[below, font=\tiny, text=gray] at (3.65, -3*0.58-0.3) {$N \times 1$};
      \end{tikzpicture}
      \end{center}
    \end{column}
  \end{columns}
...
\begin{frame}{Descente de Gradient — Intuition Géométrique}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      On cherche $\w^* = \arg\min_\w \Loss(\hat{\y}, \y)$.
...
\begin{frame}{L'Algorithme — Descente de Gradient (Batch)}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \begin{block}{Algorithme}
        \smallskip
        \begin{tabular}{@{}ll}
          \textbf{Entrée} & $\X \in \R^{N \times D}$, $\y \in \R^N$, $\lr$, $\mathit{nb\_iter}$ \\[0.3em]
          \textbf{Init}   & $\theta \leftarrow \theta_0$ \\[0.5em]
          \multicolumn{2}{@{}l}{\textbf{Répéter} $\mathit{nb\_iter}$ \textbf{fois} :} \\[0.2em]
          \hspace{1.5em}  & $\bm{g} \leftarrow \nabla_\theta\,\Loss(f_\theta(\X),\, \y)$ \\[0.2em]
                          & $\theta \leftarrow \theta - \lr\,\bm{g}$ \\[0.5em]
          \multicolumn{2}{@{}l}{\textbf{Retourner} $\theta$}
        \end{tabular}
        \smallskip
      \end{block}
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{tabular}{lp{3.2cm}}
        ...
        SGD      & Gradient sur \textbf{1 échantillon} aléatoire \\[0.3em]
