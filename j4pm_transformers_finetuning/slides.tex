...
\begin{frame}[fragile]{Anatomie du Fine-Tuning}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
...
\begin{frame}[fragile]{Hugging Face --- Le Hub \& \texttt{datasets}}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \begin{block}{The Hub --- le GitHub des modèles}
        \begin{itemize}
          \item $> 900\,000$ modèles prêts à l'emploi
          \item Hébergement gratuit des poids
          \item Versionning, fiches modèles (\textit{Model Cards}),
                benchmarks intégrés
          \item Chargement direct depuis le code :
        \end{itemize}
...
\end{lstlisting}
    ...
    \end{column}
    \begin{column}{0.46\textwidth}
      \begin{exampleblock}{\texttt{datasets} --- toutes les données}
        Accès instantané à des milliers de bases :
        \begin{itemize}
          \item IMDb, Wikipedia, GLUE, SQuAD\ldots
          \item Streaming pour les grands corpus
          \item Compatibles PyTorch et TensorFlow
        \end{itemize}
      \end{exampleblock}
\begin{lstlisting}[language=Python, basicstyle=\tiny\ttfamily]
...
# DatasetDict({train: 67349, ...})

print(ds["train"][0])
# {'sentence': 'hide new secrets',
#  'label': 0}
\end{lstlisting}
    \end{column}
  \end{columns}
...
\begin{frame}{Hugging Face --- \texttt{transformers} \& écosystème}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \begin{block}{\texttt{transformers} --- interface unifiée}
        Même API quel que soit l'architecture :
        \begin{itemize}
          \item \textbf{Encodeurs} : BERT, RoBERTa, CamemBERT
          \item \textbf{Décodeurs} : GPT-2, LLaMA, Mistral
          \item \textbf{Seq2Seq} : T5, BART, Whisper, CLIP\ldots
        \end{itemize}
        \vspace{4pt}
        \texttt{Auto*} : détecte l'architecture automatiquement.
      \end{block}
      \smallskip
      \begin{exampleblock}{Chargement universel}
        \texttt{AutoTokenizer}, \texttt{AutoModel},\\
        \texttt{AutoModelForSequenceClassification},\\
        \texttt{AutoModelForCausalLM}\ldots
      \end{exampleblock}
    \end{column}
    \begin{column}{0.46\textwidth}
      \begin{alertblock}{Écosystème complet}
        \begin{itemize}
          \item \texttt{tokenizers} : tokeniseurs ultra-rapides (Rust)
          \item \texttt{evaluate} : métriques standardisées
                (accuracy, F1, BLEU\ldots)
          \item \texttt{accelerate} : multi-GPU/TPU sans changer le code
          \item \texttt{PEFT} : fine-tuning efficace (LoRA, QLoRA)
          \item \texttt{optimum} : export ONNX, quantization
        \end{itemize}
      \end{alertblock}
      \smallskip
      \begin{block}{Vision d'ensemble}
        Hub $\to$ \texttt{datasets} $\to$ \texttt{transformers}
        $\to$ \texttt{Trainer} $\to$ Hub\\[2pt]
        Un cycle complet : charger, entraîner, publier.
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Récapitulatif --- Ce qu'on a vu}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Attention \& Transformer}
        \begin{itemize}\small
          \item Polysémie $\to$ besoin d'embeddings contextuels
          \item Mécanisme QKV : score de pertinence + softmax
          \item Multi-Head : plusieurs types de relations en parallèle
          \item Architecture : Encoder (BERT), Decoder (GPT),
                Seq2Seq (T5)
        \end{itemize}
      \end{block}
      \vspace{4pt}
      \begin{block}{Transfer Learning}
        \begin{itemize}\small
          \item Pre-training : apprendre la langue sur tout Internet
          \item Fine-tuning : adapter en quelques minutes sur une tâche
          \item Alternatives légères : LoRA, prompt-tuning, zero-shot
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Hugging Face en pratique}
        \begin{itemize}\small
          \item Hub : 900k+ modèles, versionning, Model Cards
          \item \texttt{datasets} + \texttt{transformers} : une ligne
                pour charger données et modèles
          \item \texttt{pipeline} : inférence clé en main (90\,\% des cas)
          \item \texttt{Trainer} : boucle d'entraînement complète
          \item Écosystème : tokenizers, evaluate, accelerate, PEFT
        \end{itemize}
      \end{block}
      \vspace{4pt}
      \begin{block}{LLM modernes}
        \begin{itemize}\small
          \item RLHF : aligner le modèle sur les préférences humaines
          \item Prompt engineering : zero-shot, few-shot, CoT
          \item RAG : ancrer les réponses dans des documents réels
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
