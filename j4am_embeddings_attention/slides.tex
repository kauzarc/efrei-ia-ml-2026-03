...
\begin{frame}{Récapitulatif --- Ce qu'on a vu}
  \begin{columns}[T]
    ...
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Word2Vec \& propriétés}
        \begin{itemize}\small
          \item Hypothèse distributionnelle
          \item Skip-gram : $\W_e$ comme produit dérivé
          \item Analogies vectorielles (roi $-$ homme $+$ femme)
          \item Similarité cosinus $\in [-1,\,1]$
          \item t-SNE : visualisation des clusters sémantiques
          \item Polysémie : limite des embeddings statiques
          \item[$\to$] Embeddings contextuels (J4-PM)
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
